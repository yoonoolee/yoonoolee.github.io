...
const profile = {
    ...
    links: [
        {
            icon: '<path fill="currentColor" d="M2 2.5A2.5 2.5 0 014.5 0h8.75a.75.75 0 01.75.75v12.5a.75.75 0 01-.75.75h-2.5a.75.75 0 110-1.5h1.75v-2h-8a1 1 0 00-.714 1.7.75.75 0 01-1.072 1.05A2.495 2.495 0 012 11.5v-9zm10.5-1V9h-8c-.356 0-.694.074-1 .208V2.5a1 1 0 011-1h8zM5 12.25v3.25a.25.25 0 00.4.2l1.45-1.087a.25.25 0 01.3 0L8.6 15.7a.25.25 0 00.4-.2v-3.25a.25.25 0 00-.25-.25h-3.5a.25.25 0 00-.25.25z"></path>',
            text: "UC Berkeley",
            url: "https://berkeley.edu"
        },
        {
            icon: '<path fill="currentColor" d="M12 7V3H2v18h20V7H12zM6 19H4v-2h2v2zm0-4H4v-2h2v2zm0-4H4V9h2v2zm0-4H4V5h2v2zm4 12H8v-2h2v2zm0-4H8v-2h2v2zm0-4H8V9h2v2zm0-4H8V5h2v2zm10 12h-8v-2h2v-2h-2v-2h2v-2h-2V9h8v10zm-2-8h-2v2h2v-2zm0 4h-2v2h2v-2z"/>',
            text: "Cherrytree",
        ...
        {
            icon: '<path fill="currentColor" d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/>',
            text: "Email",
...
function generateSidebar() {
    const linksHTML = profile.links.map(link => `
        <a href="${link.url}" class="profile-link" ${link.url.startsWith('http') ? 'target="_blank"' : ''}>
            <svg height="16" width="16" viewBox="0 0 ${link.icon.includes('M24') || link.icon.includes('M20') || link.icon.includes('M22') ? '24 24' : '16 16'}">${link.icon}</svg>
            <span>${link.text}</span>
        </a>
    `).join('');
...
function generateNav(currentPage) {
    return navPages.map(page =>
        `<a href="${page.url}"${page.url === currentPage ? ' class="active"' : ''}>${page.name}</a>`
    ).join('\n                ');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sidebar) sidebar.innerHTML = generateSidebar();
